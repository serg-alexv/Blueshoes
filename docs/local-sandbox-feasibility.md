...
- **Docker**: Requires significant RAM overhead. Running it on 512MB RAM alongside standard OpenWrt routing processes risks Memory Exhaustion and OOM Killer events, which breaks standard router functionality.
...
**Conclusion**: All advanced diagnostic tools must live externally on the `bs-workbench`. The router itself runs only the bare-metal OpenWrt native `bs-edge-agent`.
